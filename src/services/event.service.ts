...
export const getEventsBySearchRequest = async (request: SearchEventRequest) => {
  const {
    title,
    longitude,
    latitude,
...
export const createEvent = async (hostId: number, data: CreateEventRequest) => {
...
export const updateBoothBooking = async (id: number, sessionId: string) => {
...
export const getEventsByHostId = async (hostId: number, page: number = 1, limit: number = 20, status?: EventStatus, search?: string) => {
  const skip = (page - 1) * limit;
  const where: Prisma.eventsWhereInput = {
    host_id: hostId,
  };
...
export const getEventBySlug = async (slug: string, statuses: EventStatus[] = [EventStatus.PUBLISHED]) => {
...
export const confirmUpdateBoothStatus = async (boothId: number, status: BoothType) => {
