...
export async function bookingCleanupHandler(req, res) {
    if (req.method !== 'GET') {
...
export async function emailRetryHandler(req, res) {
    if (req.method !== 'GET') {
        return res.status(405).json({ error: 'Method not allowed' });
    }
    const authHeader = req.headers?.['authorization'] || '';
...
export async function currencyUpdateHandler(req, res) {
    if (req.method !== 'GET') {
        return res.status(405).json({ error: 'Method not allowed' });
    }
    const authHeader = req.headers?.['authorization'] || '';
...
export async function emailSyncHandler(req, res) {
    if (req.method !== 'GET') {
        return res.status(405).json({ error: 'Method not allowed' });
    }
    const authHeader = req.headers?.['authorization'] || '';
